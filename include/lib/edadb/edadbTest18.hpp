...
template<typename T>
    class DbMap {
      ...
      public:
        ...
        bool createTable(std::string tab_name,std::string fk_col="", std::string fk_ref="");
        bool insertToDb(T *obj, bool self_only = true);
        ...
        void* getSub(VecMetaData<T>::VecId id);
    ...
    template<typename T>
    void* DbMap<T>::getSub(VecMetaData<T>::VecId id){
        int i = (int)id;
        assert(0 <= i && i < VecMetaData<T>::num);
        return array_sub[i];
    }
    template<typename T> 
    bool DbMap<T>::createTable(std::string tab_name,std::string fk_col, std::string fk_ref){
        array_sub.resize(VecMetaData<T>::num);
        table_name = tab_name;
        const static std::string sql = fmt::format(SqlString<T>::createTableStr(fk_col,fk_ref), table_name);// 加两个参数
        #ifdef EDADB_DEBUG
        ...
        try {
            ...
            // createTable4Vectors();
            // if constexpr(IsWithVec<T>::value){
            //     T * t = new T;
            //     auto vec_vals = VecMetaData<T>::getVecVal(t);
            //     boost::fusion::for_each(vec_vals,VecTabHelper::createVecTable<T>(t,table_name));
            // }
    ...
    template<typename T>
    bool DbMap<T>::insertToDb(T *obj, bool self_only){
...
namespace VecTabHelper{
    ...
    struct createVecTable{
      private:
        ...
        std::string table_name;
      public:
        createVecTable(T *t,const std::string &tab_name): obj(t), table_name(tab_name) {}
        template <typename O>
            void operator()(O const& x) 
            {
                ...
                temp_dbmap.createTable("",table_name);
            }
    };
