...
namespace edadb{\
    template<>\
    struct CppTypeToVecType<myclass>{\
        static const VecTypes ret = VecTypes::kVector;\
    };\
    ...
    public:
        struct fill{
            ...
            void operator()(O val) {
                ...
                // using ObjType = typename std::remove_pointer<std::decay_t<decltype(*val)>>::type; // 考虑ptr成员
                using NakedType = std::decay_t<decltype(*val)>;
                constexpr bool is_ptr = std::is_pointer_v<NakedType>;
                using ObjType = /*NakedType;*/std::conditional_t<is_ptr, 
                    std::remove_pointer_t<NakedType>,
                    NakedType
                >;
                // std::cout<<"cnt = "<<cnt <<" "<<"ObjType =  "<<typeid(ObjType).name()<<"\n";
                if (cnt == 1){
        ...
        : sql(sql), cnt(0) {}
            ...
            void operator()(O const& x) 
            {
                ...
                using ObjType = typename std::remove_pointer<typename std::remove_const<typename std::remove_pointer<typename O::first_type>::type>::type>::type;
...
template <typename T>
    std::string SubObjVal(T* obj){
        ...
        if constexpr (std::is_class<T>::value && (!std::is_same<T, std::string>::value)){
            ...
            if constexpr (dbType == DbTypes::kComposite){
                // // if constexpr(is_ptr){
                // //     using T_Remove_Ptr = typename std::remove_pointer_t<T>;
                // //     const auto vals = TypeMetaData<T_Remove_Ptr>::getVal(*obj);
                // // }
                // // else
                // //     const auto vals = TypeMetaData<T>::getVal(obj);
                // using ValType = std::conditional_t<
                //     std::is_pointer_v<T>,
                //     decltype(TypeMetaData<std::remove_pointer_t<T>>::getVal(std::declval<std::remove_pointer_t<T>*>())),
                //     decltype(TypeMetaData<T>::getVal(std::declval<T*>()))
                // >;

                // ValType vals;

                // if constexpr (is_ptr) {
                //     using NakedType = std::remove_pointer_t<T>;
                //     vals = TypeMetaData<NakedType>::getVal(*obj);
                //     std::cout<<"getVal(*obj)\n";
                // } else {
                //     vals = TypeMetaData<T>::getVal(obj);
                //     std::cout<<"getVal(obj)\n";
                // }
                // // const auto vecs = TypeMetaData<T>::tuple_type_pair();
...
struct GetAllObjects {
    ...
    public:
        ...
        void operator()(O& x){
            ...
            using NakedType = std::remove_pointer_t<std::decay_t<decltype(x)>>;
            constexpr bool is_ptr = std::is_pointer_v<NakedType>;
            // std::cout<<"NakedType = "<<typeid(NakedType).name()<<"\n";
            using ObjType = /*NakedType;*/std::conditional_t<is_ptr, 
                std::remove_pointer_t<NakedType>,
                NakedType
            >;
...
template<typename T>
        ...
        void* DbMap<T>::getSub(int id)
    {
        ...
        if constexpr(CppTypeToVecType<T>::ret==VecTypes::kVector){
            int i = (int)id;
