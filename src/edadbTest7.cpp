...
#include "lib/edadb/edadbTest7.hpp"
/*
...
TABLE4CLASSWVEC( IdbSites, "IdbSites_all_table", (name), (idbsite_array, array2_array) );
...
int main() {
  ...
  // db.connectToDb("db=sqlite.db foreign_keys=1");
  db_IdbSite.createTable("IdbSite_table");
  // db.createTable("IdbSites_table");
  db.createTableAll("IdbSites_all_table");
  ...
  db.insertToDb_idbsite_array(p6,p1);
  db.insertToDb_idbsite_array(p6,p2);
  db.insertToDb_idbsite_array(p6,p3);
  ...
  db.updateDb_idbsite_array(p6, p1);
  db.updateDbAll(p6);
  ...
  //   std::cout << "\n-----------test selectFromDbAll-------------\n";
    ...
    db.selectFromDbAll(&result, "name = 'Sites1'"); // 这里不是 IdbSites_name = 
